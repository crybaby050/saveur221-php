...
/*
 * Petite fonction locale pour générer les badges icônes des cartes,
 * cohérente avec icone_menu() du layout mais sans lui être couplée —
 * ce jeu d'icônes est propre au dashboard.
 */
function icone_stat(string $nom): string
{
    $icones = [
        ...
        'horloge' => '<circle cx="12" cy="12" r="9"/><path d="M12 7v5l3.5 2" stroke-linecap="round" stroke-linejoin="round"/>',
...
$carteStat = static function (string $icone, string $label, string $valeur) {
    ?>
    <div class="flex items-start gap-4 rounded-xl border border-creme bg-white p-5 shadow-sm transition-shadow hover:shadow-md">
        <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-bordeaux/10 text-bordeaux">
...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <?= $carteStat('argent', "Chiffre d'affaires — jour", $formaterMontant($statistiques->chiffreAffairesJour)) ?>
        <?= $carteStat('calendrier', "Chiffre d'affaires — 7 derniers jours", $formaterMontant($statistiques->chiffreAffairesSemaine)) ?>
        <?= $carteStat('calendrier', "Chiffre d'affaires — mois", $formaterMontant($statistiques->chiffreAffairesMois)) ?>
    </div>

    <!-- Cartes activité -->
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <?= $carteStat('commandes', 'Commandes enregistrées', (string) $statistiques->nombreCommandes) ?>
        <?= $carteStat('sablier', 'Commandes en cours', (string) $statistiques->commandesEnCours) ?>
    </div>
