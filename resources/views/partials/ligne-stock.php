 ...
 */
$statutProduit = match (true) {
    $produit->estEnRupture() => 'rupture',
    $produit->estStockFaible() => 'faible',
    default => 'normal',
};
?>
<div
    ...
    data-statut-stock="<?= $statutProduit ?>"
...
>
    ...
    <div class="flex flex-wrap items-center gap-2">
        <form method="post" action="/gerant/produits/<?= $produit->getId() ?>/approvisionner" class="flex items-center gap-1.5">
            ...
            >
            <button type="button"
                onclick="demanderApprovisionnement(this.closest('form'), '<?= View::e($produit->getLibelle()) ?>')"
                class="h-9 rounded-md bg-bordeaux px-3 text-xs font-medium text-ivoire transition-colors hover:bg-bordeaux-sombre">
                Approvisionner
            </button>
        </form>
        ...
        <form method="post" action="/gerant/produits/<?= $produit->getId() ?>/seuil-alerte" class="flex items-center gap-1.5">
            ...
            >
            <button type="button"
                onclick="demanderChangementSeuil(this.closest('form'), '<?= View::e($produit->getLibelle()) ?>')"
                class="h-9 rounded-md border border-creme px-3 text-xs font-medium text-charbon transition-colors hover:bg-ivoire">
                Seuil
            </button>
        </form>
    </div>
